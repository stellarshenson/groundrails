"""R8-H79 - DANN: an N-way domain discriminator through a gradient reversal layer.

Pre-registered in docs/experiments/semantic-grounding-experiments.md (round 8).

The heretical bet of the adversarial arm: the problem is not that the loss is
averaged over domains (that is R8-H81's claim) but that the learned features
ENCODE domain. A grounding head that has internalised "this looks like a
financial table" or "this looks like Wikipedia prose" will not transfer to a
domain it has never met, which is exactly what R8-H77 measured. DANN (Ganin &
Lempitsky 2016) forces the trunk to be domain-invariant by training a domain
discriminator against it through a gradient reversal layer: the discriminator
learns to tell domains apart, the trunk learns to make that impossible.

Two domains would be domain ADAPTATION; this is multi-source domain
GENERALISATION, so the discriminator is N-way over the corpus of origin
(private / ragtruth_en / ragtruth_<lang> / halueval / psiloqa / vitaminc) rather
than binary.

Lever - the adversarial objective only. The recipe, the data mix and the data
are byte-identical to R8-H84, so any delta against H84's blind-arena 0.6450 is
attributable to the gradient reversal and nothing else.

Lambda is the load-bearing hyperparameter and the sweep is part of the
experiment, not a follow-up. An adversarial term strong enough to erase domain
also erases meaning: the expected failure is a trunk that is beautifully
domain-invariant and grounding-blind, with the discriminator at chance AND task
AUC collapsed. The diagnostic, stated before the run: the discriminator should
decay toward 1/n_groups = 1/11 = 0.091 while task AUC holds; a discriminator
BELOW chance means the reversal has pushed the trunk into anti-predicting, which
is its own pathology. Lambda is ramped in over training per the standard DANN
schedule so the trunk learns the task before it is asked to forget domain.

Re-registered bar: blind-arena mean >= 0.6450 with our gold holding >= 0.84.
Scored separately through the R8-H77 `--model` gate.

Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R8-H79_dann.py
"""
...
CKPT_DIR = HERE.parent.parent / "models" / "R8-H79-mmbert-dann"
OUT = HERE / "R8-H79_result.json"
...
# DANN: the adversarial weight. LAMBDA_MAX is the ceiling of the ramp; the ramp
# itself (Ganin's schedule) brings it in over training so the task is learned
# first. This run uses the standard ramp; a lambda sweep is the follow-up if the
# single point lands close.
LAMBDA_MAX = 0.1
DANN_HIDDEN = 256
...
def main():
    ...
    print("=" * 96)
    print("R8-H79 RESULT - DANN domain-invariant, one sub-400M model")
    print("=" * 96)
