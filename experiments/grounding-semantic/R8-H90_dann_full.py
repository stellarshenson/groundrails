"""R8-H90 - full-corpus DANN: every corpus but RAGBench, gradient reversal at 0.02.

Pre-registered in docs/experiments/semantic-grounding-experiments.md (round 8).

Directed by the author after the R8-H79 v1 read: "lets use all training data but
RagBench, and DANN is very important - it is the only way to uncover the actual
geometry of contradictions and hallucination." Two levers move together by that
directive, so the attribution control is R8-H91: an ERM twin on the IDENTICAL
full mix, differing by exactly the adversarial objective.

Changes vs R8-H79 v1, both pre-registered:

  - FULL corpora, no sub-sampling: every private train pair (~74k after the
    held-out trace filter), all of RAGTruth EN train (15k) and its seven
    translations (106k), all of HaluEval qa+summarization (40k pairs), all of
    PsiloQA train (64k), all of VitaminC train (371k), plus TabFact train (93k,
    the R8-H87 tabular lever folded in here) - ~760k pairs, 13 domain groups
  - LAMBDA_MAX 0.02, the registered v2 lever: at 0.1 the trunk anti-predicted
    (domain-acc 0.000 vs chance 0.083, domain loss rising). True invariance
    parks the discriminator AT chance; the mid-run health check, stated before
    the run, is domain-acc in [0.03, 0.15] once the ramp saturates. Anti-
    prediction reappearing at 0.02 kills the lambda lever and points at v3
    (stronger discriminator)

RAGBench remains excluded - the arena stays blind.

Bar: blind-arena mean >= 0.6461 (beat the incumbent). Guardrail: all three
in-domain bars beaten, gold >= 0.80 (relaxed from 0.84 BEFORE the run: private
falls from 24% to ~10% of the mix; the 0.84 line is still reported).

Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R8-H90_dann_full.py
"""
...
CKPT_DIR = HERE.parent.parent / "models" / "R8-H90-mmbert-dann-full"
OUT = HERE / "R8-H90_result.json"
...
def main():
    ...
    print("=" * 96)
    print("R8-H90 RESULT - full-corpus DANN, one sub-400M model")
    print("=" * 96)
