"""R9-H105 draw 2 - multi-seed protocol: second draw of the clean-mix recipe.

Pre-registered in docs/experiments/semantic-grounding-experiments.md (round 9).

Identical recipe, data, and budget to R9-H105 draw 1 (SEED 0 keeps the same
gold-trace eval split; model init and batch order are unseeded, so a rerun
samples the run-to-run noise of the clean configuration - the R8-H100
precedent). Only the artifact paths differ.
...
Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R9-H105_draw2.py
"""
...
CKPT_DIR = HERE.parent.parent / "models" / "R9-H105-draw2"
OUT = HERE / "R9-H105_draw2_result.json"
...
def main():
    ...
    print("=" * 96)
    print("R9-H105 DRAW 2 RESULT - clean-mix recipe, second draw (private pairs removed)")
    print("=" * 96)
