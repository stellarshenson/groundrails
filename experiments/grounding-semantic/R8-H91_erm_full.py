"""R8-H91 - full-corpus ERM: the attribution control for R8-H90.

Pre-registered in docs/experiments/semantic-grounding-experiments.md (round 8).

R8-H90 moves two levers at once by the author's directive (full corpora + DANN).
This twin trains on the IDENTICAL ~760k-pair mix - every corpus but RAGBench,
TabFact included - with plain ERM (BCE, no discriminator). H90 minus H91 is the
adversarial objective's contribution at full scale; H91 minus H84 is the data's.
...
Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R8-H91_erm_full.py
"""
...
CKPT_DIR = HERE.parent.parent / "models" / "R8-H91-mmbert-erm-full"
OUT = HERE / "R8-H91_result.json"
...
def main():
    ...
    print("=" * 96)
    print("R8-H91 RESULT - full-corpus ERM control, one sub-400M model")
    print("=" * 96)
