"""R8-H99 - full-corpus DANN from scratch at the H93 winner lambda (0.1241).

Pre-registered in docs/experiments/semantic-grounding-experiments.md (round 8).

The single-variable twin of R8-H90: identical recipe, data (full 762k
natural-frequency mix), and budget - only LAMBDA_MAX moves, 0.02 -> 0.1241
(the H93 LOCO sweep winner). H99 vs H90 isolates lambda at full scale;
H99 vs H96 isolates the trunk init behind the curriculum's failure.

Bar: blind decomposed-min >= 0.7313 (H90 + 0.01), >= 7/10 subsets; guardrail
3 in-domain bars beaten, gold >= 0.80. Anti-prediction is a recorded
diagnostic, not a kill (H93 evidence); kill = task-loss divergence only.

Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R8-H99_dann_highlambda.py
"""
...
CKPT_DIR = HERE.parent.parent / "models" / "R8-H99-mmbert-dann-highlambda"
OUT = HERE / "R8-H99_result.json"
...
def main():
    ...
    print("=" * 96)
    print("R8-H99 RESULT - full-corpus DANN at the H93 winner lambda")
    print("=" * 96)
