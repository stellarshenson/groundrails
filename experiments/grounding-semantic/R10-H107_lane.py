"""R10-H107 - procedural-doc-register lane: the clean recipe plus the H107 pairs.
...
Run:  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
      uv run python experiments/grounding-semantic/R10-H107_lane.py --draw 1
"""
...
LANE = DATA / "R10-H107_pairs.parquet"
...
def lane_train():
    """The H107 lane pairs, already provenance-gated. Each lane source (`tag`)
...
def main():
    ...
    args = ap.parse_args()
    ckpt_dir = HERE.parent.parent / "models" / f"R10-H107-lane-draw{args.draw}"
    out = HERE / f"R10-H107_lane_draw{args.draw}_result.json"
    ...
    chance = 1.0 / n_groups
    print(
        f"train: {len(y)} pairs ({n_public} clean + {len(ly)} H107 lane) across "
    ...
    res["lane"] = "R10-H107 procedural-doc-register"
    res["draw"] = args.draw
    res["mix"] = "clean public-only mix + H107 lane"
    ...
    print("=" * 96)
    print(f"R10-H107 LANE DRAW {args.draw} - lineage reads (admission is the blind arena)")
    print("=" * 96)
